...
export default function SuccessPage() {
  ...
  // fetch dettagli della sessione (server API sicura che hai già)
  useEffect(() => {
    let cancelled = false;
    async function run() {
      if (!sessionId) {
      ...
      try {
        const base =
          ...
          '';
        const res = await fetch(`${base}/api/checkout/success?session_id=${encodeURIComponent(sessionId)}`, { cache: 'no-store' });
  ...
  return (
    ...
    <main className="relative isolate min-h-[100dvh] overflow-hidden text-white">
      {/* BACKGROUND — riprende la tua Hero */}
      ...
      {/* CONFETTI responsive (si spengono dopo 5s, rispettano reduce-motion) */}
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-16 sm:py-20 md:py-24">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            ...
            {/* dettagli ordine */}
            <motion.ul className="mt-8 space-y-1 text-sm text-neutral-300" {...fade(3)}>
              {loading && <li className="opacity-80">Ładuję szczegóły…</li>}
              {!loading && data?.email && (
                <li>
                  <span className="text-neutral-400">E-mail:</span> {data.email}
                </li>
              )}
              {!loading && data?.subscriptionStatus && (
                <li>
                  <span className="text-neutral-400">Status subskrypcji:</span> {data.subscriptionStatus}
                </li>
              )}
            </motion.ul>
            ...
            {/* mobile image sotto (come nella Hero) */}
          ...
          {/* immagine destra (coerente con Hero) */}
        ...
        {/* Card “glass” in stile pricing, come highlight finale */}
        <motion.div
      ...
      {/* accento conico (emerald/amber) come nella Hero */}
      <div
        aria-hidden
